    ...
    <div style={{maxWidth:1080, margin:"0 auto"}}>
      ...
      {error && (
        <div className="card" style={{padding:"10px 14px", marginBottom:14, borderLeft:"3px solid var(--pink-500)", fontSize:13}}>
          Couldn't load your workspace stats — {error}
        </div>
      )}
          ...
          <div className="card" style={cardStyle}>
            ...
            <div style={{display:"flex", alignItems:"baseline", gap:8}}>
              ...
              <span style={{fontFamily:"var(--font-mono)", fontSize:11, color:"var(--c-faint)"}}>
                ...
                {credits?.monthly ? `/ ${credits.monthly} · ` : ""}{daysLeftInCycle()}d left
              </span>
            </div>
        ...
        <Reveal delay={80}>
          <div className="card" style={cardStyle}>
            ...
            <div style={{display:"flex", alignItems:"baseline", gap:20}}>
              <div><div style={big()}>{unknown ? "—" : briefs.length}</div><div className="eyebrow" style={{marginTop:5}}>created</div></div>
              <div><div style={big("var(--green-600)")}>{unknown ? "—" : `${rate}%`}</div><div className="eyebrow" style={{marginTop:5}}>success rate</div></div>
            </div>
            <div style={{fontSize:12, color:"var(--c-faint)"}}>{unknown ? "—" : `${inFlight.length} in flight · ${shipped} shipped`}</div>
          </div>
        </Reveal>
        <Reveal delay={160}>
          <div className="card" style={cardStyle}>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:7, marginTop:2}}>
              ...
              {!unknown && recent.length === 0 && (
                <div style={{fontSize:12.5, color:"var(--c-faint)"}}>Nothing yet — your first brief fills this.</div>
              )}
              {error && <div style={{fontSize:12.5, color:"var(--c-faint)"}}>Couldn't load.</div>}
              {recent.map(o => (
            ...
            {inFlight.map(b => {
              ...
              return (
                <a key={b.id} href={"#/board/" + b.id} className="card" style={{padding:16, textDecoration:"none", color:"inherit", cursor:"pointer", display:"flex", flexDirection:"column", gap:7}}>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10}}><span className="eyebrow">{b.type === "one_off" ? "Brief" : (b.type || "Brief")}</span><StatusPill status={running ? "in-production" : "review"} /></div>
                  <div style={{fontSize:14.5, fontWeight:500, color:"var(--c-ink)", letterSpacing:"-0.005em"}}>{b.title || b.payload?.title || "Untitled brief"}</div>
                  ...
                  <div className="card" style={{padding:"20px 24px", boxShadow:"var(--shadow-md)"}}>
                    ...
                    <div style={{
                      ...
                      <div style={{fontFamily:"var(--font-mono)", fontSize:12, color:"var(--c-dim)"}}>
                        ...
                        {stats.credits
                          ? `${stats.credits.balance - (phase === "done" ? realAssembly.totalCr : 0)} remaining after`
                          : "balance loading…"}
                      </div>
                      {phase === "proposing" && (
